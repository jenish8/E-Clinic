...
export class patientservice{
    ...
    getAllPatient():Observable<any[]>{
        return this.http.get<any[]>(api+`/appointment`,{headers:new HttpHeaders({'Content-Type':'application/json'})})
        .map(res => res);
    }

    createPatient(body:any):Observable<any[]>{

        return this.http.post<any[]>(api+`/add_appointment`,body,{headers:new HttpHeaders({'Content-Type':'application/json'})})
        .map(res => res);
    }

    getSiteById(id:any):Observable<any[]>{
        return this.http.get<any[]>('http://localhost:3000/patient_details/onepatient/'+id,{headers:new HttpHeaders({'Content-Type':'application/json'})})
        .map(res => res);
    }
